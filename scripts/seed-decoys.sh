...
# Read the registry FIRST, into a variable, so a failure is fatal. Piping
# straight into the loop below via `< <(...)` hides it: process substitution
# exit status is invisible to set -e and pipefail, so an unreadable registry
